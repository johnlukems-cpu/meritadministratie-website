...
/**
 * Social-media-links. Toont alleen kanalen waarvan een URL in siteConfig.social staat;
 * zonder URL's rendert dit component niets (geen nep-links).
 */
const icons = {
  linkedin: {
    label: 'LinkedIn',
  ...
  facebook: {
    label: 'Facebook',
    path: 'M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.09 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.96.93-1.96 1.88v2.26h3.33l-.53 3.49h-2.8V24C19.61 23.09 24 18.1 24 12.07z',
  },
} as const;

export function SocialLinks({ className, onDark }: { className?: string; onDark?: boolean }) {
  const entries = (Object.keys(icons) as Array<keyof typeof icons>)
    .map((key) => ({ key, url: siteConfig.social[key], ...icons[key] }))
    .filter((e): e is typeof e & { url: string } => Boolean(e.url));

  if (!entries.length) return null;

  return (
    <ul className={cn('flex gap-3', className)} aria-label="Social media">
      {entries.map((e) => (
        <li key={e.key}>
          <a
            href={e.url}
            ...
            aria-label={`${siteConfig.shortName} op ${e.label}`}
            className={cn(
              'inline-flex size-10 items-center justify-center rounded-md border transition-colors',
              onDark
                ? 'border-on-dark-border text-on-dark-muted hover:border-accent hover:text-on-dark focus-visible:outline-white'
